# Confgo — Product Design Document (v4)

A specification complete enough to rebuild the application from scratch.

> **v3 → v4 changelog.** v4 closes 14 gaps flagged in review: API payload types (§8.7), decay curves (§6.6), admin-gated conference creation (§3.2, §7.1), map clustering & size scaling (§3.1), glass panel CSS (§5.4), error contract (§8.8), topic/URL display (§3.2), Unnamed vs Anonymous naming (§3.5), ping rate limits (§3.8), responsive breakpoints (§6.7), environment variables & CORS (§10, §8.9), accessibility (§6.8), attendee privacy rationale (§3.2). Attendee-list mutation (edit/delete conferences) is deferred per product — noted in §3.2.
...
- **Brightness = recency.** A conference happening now renders at full brightness. See §6.6 for the exact `conferenceGlow()` formula.
...
- **Location grouping.** Conferences are grouped by location using the composite key `round(lat, 4) + "|" + round(lng, 4)` (≈11m precision). When a group contains one conference, clicking its dot opens the detail view directly. When a group contains more than one, clicking opens the **LocationSheet** — a chronological list for that location with a "NOW" divider, auto-scrolled to the present. Past entries are dimmed. Each entry shows name, date, and your attendance status. Clicking one opens its detail view.
- **Dot size scales with group size.** For locations with multiple conferences, the core dot radius is `baseRadius + log2(count + 1) * 1.6` (px), capped at 16px. The halo scales to `coreRadius * 2.2`. This makes dense cities like NYC (61 conferences) visibly heavier than one-off locations without requiring explicit cluster markers or spiderfy behavior.

Map configuration: Mapbox `dark-v11` style, mercator projection, initial center at `[-98.35, 39.5]` zoom 3.85 (continental US). Fog config: `range: [0.8, 8]`, `color: 'rgb(9, 12, 22)'`, `high-color: 'rgb(36, 44, 72)'`, `horizon-blend: 0.18`, `star-intensity: 0.25`.
...
**Conference detail view (ConferenceSheet):** name, location, dates, attendee count, and attendee avatars/photos with ping indicators. Attendance buttons: "I'll be there" / "I was there" / "Unmark." Topics render as small pill chips below the date row (font 0.68rem, uppercase, `--mist` border). URL, if present, renders as a quiet link "Visit website ↗" below the topic row (external, `target="_blank" rel="noopener noreferrer"`).

**Attendee-list privacy.** The `/conferences/:id/attendees` endpoint is intentionally readable without auth. Confgo's identity model — avatar glyph or profile photo plus optional display name — is the entirety of a user's public surface. No email, real name, company, or contact information is ever exposed via the API. An attendee list is indistinguishable from a list of avatars, which is the design intent. Logged-in callers additionally receive `youPinged`/`hasPingedYou` flags for UI affordances.
...
**Adding conferences (admin-gated).** `POST /conferences` is accessible only to users whose Firestore user doc has `is_admin: true`. The "Add a conference" button in Settings is hidden for non-admins. Admin promotion is a manual Firestore console action in v1 — no self-serve admin request flow, no admin-management UI. Zod validates: name (1–200 chars), location name (1–200), latitude (-90 to 90), longitude (-180 to 180), start/end dates (ISO 8601 with offset), end ≥ start. Non-admin callers receive a 403.

**Editing / deleting conferences is deferred.** Extended conference admin (edit, delete, merge, moderation) is a v2 effort. Conferences in v1 are append-only.
...
**Profile photos:** Upload from device, crop and position within a circular preview using drag + zoom slider, client-side resize to 200×200 JPEG (0.85 quality) via `<canvas>`, uploaded directly to Firebase Cloud Storage at `profile-images/{userId}/photo.jpg`. Photos replace the glyph everywhere in the UI. "Remove photo" reverts to glyph.

**Display names:** Optional free-text (max 50 chars), searchable. Name-rendering rules:
- **"Unnamed"** — rendered in any third-party context (attendee lists, peers, search results, ping entries) when the target user has `display_name == null`. Applies to both authenticated and anonymous targets — from another user's perspective, a no-name user is a no-name user.
- **"Anonymous"** — rendered *only* in the viewer's own Settings panel subtitle when the viewer is signed in via Firebase Anonymous Auth (no linked credential). This is a cue to the viewer that their account is not yet recoverable, not a visible label elsewhere.
...
Search is case-insensitive substring match server-side (in-memory filter across the users collection; acceptable for <10k users).
...
**Ping rate limits** (server-enforced, returns 429 with a `Retry-After` header in seconds):
- Max **10 pings per rolling hour** per sender (`PING_RATE_LIMIT_HOUR`)
- Max **50 pings per rolling day** per sender (`PING_RATE_LIMIT_DAY`)
- After a ping is **rejected**, the sender cannot re-ping the same recipient for 30 days (the rejection's decay window). Attempting to re-send returns 409.
- **Revoking** a ping does not count against the rate limit retroactively but re-sending after revoke does.
...
- Manage section (divider above, visible only to `is_admin: true` users): "Add a conference" button
...
- **Crop:** 180px circular preview with drag-to-reposition, zoom slider (1×–3×). Save renders to 200×200 canvas, uploads to Cloud Storage.
...
- Glass panel aesthetic (exact CSS):
  ```css
  background: linear-gradient(135deg, rgba(12, 16, 32, 0.88) 0%, rgba(3, 4, 10, 0.92) 100%);
  backdrop-filter: blur(18px) saturate(140%);
  -webkit-backdrop-filter: blur(18px) saturate(140%);
  border: 1px solid var(--mist);
  border-radius: 14px;
  box-shadow: 0 24px 64px -16px rgba(0, 0, 0, 0.7), inset 0 1px 0 rgba(255, 255, 255, 0.04);
  ```
...
--text-muted: rgba(232, 240, 255, 0.55)   Secondary text
```
...
- **Core dot:** radius 3.5–9px by glow × location-group factor, blur 0.25. Color by state (mine/past/default).
- **Halo ring:** radius 7–18px by glow × location-group factor, blur 0.9, lower opacity. Same color logic.
...
- `sheet-in`: 420ms cubic-bezier(0.2, 0.9, 0.2, 1) slide-up + fade for panel entry
- `ping-pulse`: 2s ease-in-out infinite opacity oscillation (1 → 0.4 → 1) for incoming ping rings
- Map fly-to: Mapbox `flyTo` with `duration: 1100, curve: 1.4, speed: 0.9, essential: true`
- All animations respect `prefers-reduced-motion: reduce` — replaced with instant state change.

### 6.5 Iconography

All icons inline SVG, 24×24 viewBox, stroke-based:
- Signal (radio-wave chevrons — ping inbox)
- Gear (settings)
- Calendar (my conferences)
- Crosshair (co-attendance)
- Pencil (edit)
- X-circle (close)
- Arrow-left (back)
- Plus (add)

Defined once in `web/src/components/icons.tsx` and re-used. No icon fonts, no sprite sheets.

### 6.6 Decay Curves

**Conference glow.** `conferenceGlow(startDate, endDate, now) → [0.08, 1.0]`:

```
if (now >= startDate && now <= endDate) return 1.0;       // during event
const daysAway = (now < startDate)
  ? (startDate - now) / 86_400_000
  : (now - endDate) / 86_400_000;
const halfLifeDays = 180;
const raw = Math.pow(2, -daysAway / halfLifeDays);
return Math.max(0.08, raw);                               // floor so points remain visible
```

Sample values (decay from event edge): 0d→1.00, 30d→0.89, 90d→0.71, 180d→0.50, 365d→0.25, 720d→0.08 (floor).

The curve is symmetric around the event window (approaching future events ramp up the same way past events decay). Floor at 0.08 ensures old conferences remain visible on the map without dominating.

**Ping intensity.** `pingIntensity(createdAt, nowMs, decayDays=30) → [0, 1]`:

```
const ageDays = (nowMs - Date.parse(createdAt)) / 86_400_000;
return Math.max(0, 1 - ageDays / decayDays);              // linear
```

Client ring opacity: `0.35 + pingIntensity * 0.65`.

### 6.7 Responsive Breakpoints

- **Desktop (≥ 1024px):** default layout — sheets top-left (max 420px wide), toolbar top-right (max 260px), both with 24px viewport margin.
- **Tablet (768–1023px):** sheets widen to `min(calc(100vw - 48px), 480px)`; toolbar collapses the Past/Future filter row into a popover behind a funnel icon.
- **Mobile (< 768px):** sheets become bottom-sheets — `bottom: 0; left: 0; right: 0; border-radius: 18px 18px 0 0; max-height: 85vh`. Search stack collapses into a top bar with an expand-to-search tap target. Toolbar becomes a floating circular action button cluster in the bottom-right, each action opening its panel as a bottom sheet. Map fog and zoom preserved.

All panels honor iOS/Android safe-area insets via `env(safe-area-inset-*)`.

### 6.8 Accessibility

- **Keyboard.** All interactive elements reachable via `Tab`. Map receives keyboard focus via a visible "Skip to search" link that the user can use to bypass it. Every button has a visible focus ring (`outline: 2px solid var(--signal); outline-offset: 2px`).
- **Focus management.** Opening a sheet moves focus to its first interactive element; closing returns focus to the trigger. Modals (AddConferenceModal, ProfileImageEditor) use `role="dialog" aria-modal="true"` and trap focus (Tab cycles within).
- **Screen readers.** All icon-only buttons have `aria-label`. The map surfaces a visually-hidden heading ("Conference map") and live region that announces LocationSheet and ConferenceSheet open events.
- **Contrast.** Body text (`--text` on `--haze`) measures ~13.5:1. Muted text (`--text-muted` = 0.55 opacity, effective ~7.4:1) passes WCAG AA. Button borders are never the only affordance — buttons also have contrasting backgrounds on hover/focus.
- **Motion.** `prefers-reduced-motion: reduce` disables `sheet-in`, `ping-pulse`, and `flyTo` transitions (replaced with `duration: 0`).
- **Target size.** All tap targets ≥ 44×44px on mobile breakpoints (WCAG 2.2 AA).
...
users/{userId}
  ...
  is_admin          boolean (default false; grants conference create rights)
...
attendances/{attendanceId}
  ...
  intent            "been" | "going"
  created_at        string (ISO 8601)
  Uniqueness: one doc per user_id + conference_id pair (doc id: "{user_id}_{conference_id}")
...
pings/{pingId}
  ...
  Uniqueness: one doc per from_user_id + to_user_id pair (doc id: "{from}_{to}")
```
...
- **conferences:** authenticated read; **no direct writes** — all writes go through the Cloud Function which enforces `is_admin`.
- **users:** authenticated read all, write own only (`request.auth.uid == userId`), and the `is_admin` field is read-only from the client (server-side rule blocks client writes to `is_admin`).
- **attendances:** authenticated read; writes go through the Cloud Function which enforces `user_id == request.auth.uid`.
- **pings:** authenticated read; writes go through the Cloud Function.
...
Auth middleware: extracts Bearer token from Authorization header, verifies via Firebase Admin `verifyIdToken()`. Auto-provisions user doc on first API call (random avatar, null email, `is_admin: false`). Syncs email from token to Firestore if changed (handles credential linking).
...
|--------|------|------|---------------|----------|
| GET | `/me` | Yes | — | `User` |
| PATCH | `/me` | Yes | `{ avatarId?, displayName?, photoURL? }` (null to clear) | `User` |
| GET | `/me/attendances` | Yes | — | `{ attendances: Attendance[] }` |
...
| POST | `/conferences` | Yes (admin) | `NewConference` | `Conference` (201) |
| GET | `/conferences/:id` | No | — | `Conference` |
...
| Method | Path | Auth | Body / Params | Response |
|--------|------|------|---------------|----------|
| GET | `/users/search` | Yes | `?q=name` | `{ users: UserSummary[] }` (max 20) |
| GET | `/users/:userId/profile` | Yes | — | `{ user: UserSummary, conferences: Conference[], shared: Conference[] }` |
...
| Method | Path | Auth | Body / Params | Response |
|--------|------|------|---------------|----------|
| POST | `/users/:targetId/ping` | Yes (linked) | — | `{ ok: true }` — creates or refreshes ping; 429 on rate limit; 409 on rejection cooldown |
| GET | `/pings/incoming` | Yes | — | `{ incoming: IncomingPing[] }` |
| GET | `/pings/outgoing` | Yes | — | `{ outgoing: OutgoingPing[] }` |
| POST | `/pings/:pingId/ping-back` | Yes (linked) | — | `{ ok: true }` |
| POST | `/pings/:pingId/reject` | Yes (linked) | — | `{ ok: true }` |
| POST | `/pings/:pingId/revoke` | Yes (linked) | — | `{ ok: true }` — deletes the ping doc |
| POST | `/pings/dematch/:peerId` | Yes (linked) | — | `{ ok: true }` — deletes both directions |
...
All responses are JSON with `Content-Type: application/json`. Field naming in wire payloads is **camelCase** (server translates from Firestore snake_case).
...
interface Attendee extends UserSummary {
  intent: "been" | "going";
  youPinged?: boolean;       // present when caller is authenticated
  hasPingedYou?: boolean;    // present when caller is authenticated
}

interface CoPeer extends UserSummary {
  sharedCount: number;
...
interface IncomingPing {
  ...
  from: UserSummary;
  createdAt: ISO8601;
  intensity: number;           // 0–1 (server-computed using PING_DECAY_DAYS)
  mutual: boolean;             // true if recipient has also sent a ping back
}
...
interface MutualContact extends UserSummary {
  pingId: string;              // either direction's id; used for dematch routing
  matchedAt: ISO8601;          // max(a.createdAt, b.createdAt)
}
```
...
| Status | `code` | Trigger |
|---|---|---|
| 400 | `validation_failed` | Zod body/query/params validation failed. `details` = array of `{ path, message }`. |
| 401 | `unauthenticated` | Bearer token missing or invalid. |
| 403 | `forbidden` | Authenticated but not permitted (e.g., non-admin POSTing conference; anonymous attempting a linked-only action). |
| 404 | `not_found` | Resource does not exist or not visible. |
| 409 | `conflict` | Duplicate resource, ping-after-rejection cooldown, unique constraint violation. |
| 429 | `rate_limited` | Ping rate limit exceeded. Response also sets `Retry-After` header (seconds). |
| 500 | `internal` | Unexpected server error. Details omitted; logged. |

### 8.9 CORS

Configured via Hono `cors()` middleware on the root app:

```ts
origin: (origin) => {
  if (!origin) return "*";                          // allow same-origin / curl
...
Every request logs a structured line via `console.log(JSON.stringify(...))` — picked up by Cloud Logging:

```json
{ "t": "2026-04-18T14:12:03.441Z", "method": "POST", "path": "/conferences", "uid": "abc123", "status": 201, "ms": 41 }
```

Errors add `"err": { "message": "...", "stack": "..." }`.
...
| Dev environment | Firebase Emulator Suite (Auth 9399, Firestore 8380, Functions 5301, Hosting 5300, Storage 9499, UI 4300, Hub 4700, Logging 4800) — shifted +300 from defaults so multiple Firebase projects can run concurrently on the same host |

### 10.1 Environment Variables

**Web (`web/.env`):**
```
VITE_MAPBOX_ACCESS_TOKEN=pk.****        # required
VITE_USE_FIREBASE_EMULATORS=1           # optional; "1" to connect local emulators
VITE_FIREBASE_API_KEY=demo              # Firebase Web config (public)
VITE_FIREBASE_AUTH_DOMAIN=demo-confgo.firebaseapp.com
VITE_FIREBASE_PROJECT_ID=demo-confgo
VITE_FIREBASE_STORAGE_BUCKET=demo-confgo.appspot.com
...
```
PING_DECAY_DAYS=30
PING_RATE_LIMIT_HOUR=10
PING_RATE_LIMIT_DAY=50
```

**Scripts (pipeline):**
```
MAPBOX_TOKEN=pk.****                    # for collect-conferences geocoding
FIREBASE_PROJECT=demo-confgo            # override if needed
```
...
2. **Server-side aggregation for co-attendance.** Firestore doesn't support joins. Rather than denormalize, co-attendance is computed per-request in the Cloud Function. Acceptable for the current dataset size (<2000 conferences, <200 users).
...
5. **Client-side image processing.** Profile photos are cropped and resized to 200×200 in the browser via `<canvas>` before upload. No server-side image processing needed.
...
11. **Admin-gated conference creation.** v1 keeps the corpus curated. Self-serve submission and full conference CRUD are v2 concerns. The `is_admin` flag is set manually via Firestore console.

12. **Identity = avatar + display name.** No email, real name, company, or contact fields exposed via the API. The identity model is the privacy model — there is nothing else to leak.
